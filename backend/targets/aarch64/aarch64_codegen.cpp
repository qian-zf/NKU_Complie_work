...
namespace BE::AArch64
{
    ...
    void Codegen::emitInstruction(BE::MInstruction* minst)
    {
        ...
        if (minst->kind == BE::InstKind::SSLOT)
        {
            // 应该在 StackLowering 中处理，如果残留则打印注释
            out_ << "  // Skipped SSLOT instruction\n";
            return;
        }

        if (minst->kind == BE::InstKind::LSLOT)
        {
            // 应该在 StackLowering 中处理，如果残留则打印注释
            out_ << "  // Skipped LSLOT instruction\n";
            return;
        }
