...
namespace ME
{
    ...
    void ASTCodeGen::handleUnaryCalc(FE::AST::ExprNode& node, FE::AST::Operator uop, Block* block, Module* m)
    {
        ...
        // ???: ???? NOT ????????? i1 (from icmp)???????????????????? i32 (int)?????????????
        if (uop == FE::AST::Operator::NOT)
        {
            DataType expectedType = convert(node.attr.val.value.type);
            // ???[? block ???????????????????????????????? maxReg
            // opFunc (notInt/notFloat) ?????????????????????
            // ???????????? opFunc ??????????????????????? getMaxReg ???
            // ???: ??????? opFunc ????????????????????????
            
            // ??? opFunc ?? notInt???????? icmp -> i1
            // ??? expectedType ?? i32????????? zext i1 -> i32
            
            if (expectedType == DataType::I32 && srcType == DataType::I32) // srcType was I32 (int input), output is i1
            {
                size_t resReg = getMaxReg();
                // ??? resReg ?????????????? i1 (????? icmp ???)
                // ???????
                auto convInsts = createTypeConvertInst(DataType::I1, DataType::I32, resReg);
                for (auto& inst : convInsts) block->insert(inst);
            }
        }
    }
