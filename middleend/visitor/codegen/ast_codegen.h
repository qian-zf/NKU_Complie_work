...
namespace ME
{
    ...
    class ASTCodeGen : public CodeGen_t
    {
      ...
      private:
        const std::map<FE::Sym::Entry*, FE::AST::VarAttr>&       glbSymbols;  //全局变量符号表
        const std::map<FE::Sym::Entry*, FE::AST::FuncDeclStmt*>& funcDecls;   //函数声明表，便于查找函数信息
        Function*                                                curFunc;     //当前正在生成IR的函数
        Block*                                                   curBlock;    //当前正在生成IR的基本块
        class RegTab  //局部寄存器表，支持作用域嵌套
        {
          public:
            struct Scope
            {
        ...
        std::map<size_t, FE::AST::VarAttr>        reg2attr;     //寄存器变量属性
        std::map<size_t, bool>                    paramPtrTab;  // if the i-th param is a pointer or not
        std::map<FE::AST::LeftValExpr*, Operand*> lval2ptr;     //左值表达式对应的内存地址（生成store/load时用）
